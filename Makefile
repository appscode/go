...
SRC_PKGS := analytics arrays container context crypto encoding env flags homedir ioutil log net ntp os pongo2 proc reflect runtime sets signals strings sync term types util version wait
...
gen-crd-protos:
	@docker run --rm                                     \
		...
		go-to-protobuf \
			...
			--packages=github.com/appscode/go/encoding/json/types
